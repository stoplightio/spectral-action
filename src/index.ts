...
createConfigFromEnv
  ...
  .chain(({
    ...
    const getRepositoryInfoFromEvent =
      fromIOEither(tryCatch2v<string, Event>(() => require(GITHUB_EVENT_PATH), e => String(e)))
        .map(event => {
    ...
    const createOctokitInstance =
      fromIOEither(tryCatch2v(() => new Octokit({ auth: `token ${GITHUB_TOKEN}` }), e => String(e)));

    const readFileToAnalyze =
      tryCatch2v(() => readFileSync(join(GITHUB_WORKSPACE, SPECTRAL_FILE_PATH), { encoding: 'utf8' }), e => String(e))
          ...
          .chain(check =>
            fromIOEither(readFileToAnalyze
              ...
              .chain(({ parsed, results }) => {
                ...
                return tryCatch(() => octokit.checks.update({
